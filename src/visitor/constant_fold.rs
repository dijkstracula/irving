...
impl VisitorV2 for ConstantFold {
    fn finish_binop(&mut self, expr: &mut BinOp) -> VisitorV2Result {
        match (expr.lhs.as_ref(), &expr.op, expr.rhs.as_ref()) {
            (Expr::Number(lhs), Verb::Plus, Expr::Number(rhs)) => {
                Ok(ControlV2::ChangeExpr(Expr::Number(lhs + rhs)))
            }
            ...
            (lhs, Verb::Plus, Expr::Number(0)) => Ok(ControlV2::ChangeExpr(lhs.clone())),
