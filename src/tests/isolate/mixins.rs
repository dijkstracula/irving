...
mod tests {
    ...
    fn simple_action() {
        ...
        assert_eq!(decl,
            ActionDecl {
                ...
                params: vec!(Param{ id: "a".to_owned(), sort: Some(vec!("int".to_owned()))}),
                ret: Some(Param{id: "b".to_owned(), sort: Some(vec!("int".to_owned()))}),
                body: Some(vec!())
            }
        );
        ...
        assert_eq!(mixin,
            rewriter::Mixin {
                name: "foo".to_owned(),
                params: Some(vec!(Param{ id: "a".to_owned(), sort: Some(vec!("int".to_owned()))})),
                ret: Some(Param{id: "b".to_owned(), sort: Some(vec!("int".to_owned()))}),
                pre: None,
                body: Some(vec!()),
                post: None,
    ...
    fn action_and_post_correct() {
        ...
        for decl in decls {
            match decl {
                Decl::Action(ad) =>        { mixin.mix_action(ad).unwrap(); }
                Decl::AfterAction(post) => { mixin.mix_after(post).unwrap(); }
                Decl::BeforeAction(pre) => { mixin.mix_before(pre).unwrap(); }
                _ => unimplemented!()
            }
        }
